...
% Options
% ============================================= %
filename='DataRequests_MSCI_D.xlsm';  %  Return indices in local currency
...
temp=xlsread(strcat(newpath,filename),'Sheet1');
s=size(temp);
MSCI_temp=zeros(s(1),s(2),3);
% First column : Dates
MSCI_Market_temp1=xlsread(strcat(newpath,filename),'Sheet1'); % Market Return Index 1976-1986
MSCI_Growth_temp1=xlsread(strcat(newpath,filename),'Sheet2'); % Growth Return Index 1976-1986
MSCI_Value_temp1=xlsread(strcat(newpath,filename),'Sheet3');  % Value Return Index 1976-1986

MSCI_Market_temp2=xlsread(strcat(newpath,filename),'Sheet4'); % Market Return Index 1986-1996
MSCI_Growth_temp2=xlsread(strcat(newpath,filename),'Sheet5'); % Growth Return Index 1986-1996
MSCI_Value_temp2=xlsread(strcat(newpath,filename),'Sheet6'); % Value Return Index 1986-1996

MSCI_Market_temp3=xlsread(strcat(newpath,filename),'Sheet7'); % Market Return Index 1996-2009
MSCI_Growth_temp3=xlsread(strcat(newpath,filename),'Sheet8'); % Growth Return Index 1996-2009
MSCI_Value_temp3=xlsread(strcat(newpath,filename),'Sheet9'); % Value Return Index 1996-2009

MSCI_Market_temp4=xlsread(strcat(newpath,filename),'Sheet10'); % Market Return Index 2009-now
MSCI_Growth_temp4=xlsread(strcat(newpath,filename),'Sheet11'); % Growth Return Index 2009-now
MSCI_Value_temp4=xlsread(strcat(newpath,filename),'Sheet12'); % Value Return Index 2009-now
...
code_imf =[];
for j=1:size(CountryName_IMF,1)
   if strcmp(CountryName_IMF(j,1),'UNITED STATES')==1
...
NM = split(between(datetime(Barclays_MSCI_Market_D(2,1),'ConvertFrom','datenum'),datetime(Barclays_MSCI_Market_D(end,1),'ConvertFrom','datenum'), 'Months'), 'Months');
...
for t=2:size(Barclays_MSCI_Market_D,1)-1
    if month(datetime(Barclays_MSCI_Market_D(t,1),'ConvertFrom','datenum'))~=month(datetime(Barclays_MSCI_Market_D(t+1,1),'ConvertFrom','datenum')) % find end-of-month
        [N, S]=weekday(datetime(Barclays_MSCI_Market_D(t,1),'ConvertFrom','datenum'));
        j=0;
        switch N
            case 1 % Sunday
...
% =======
% Transform all series into end-of-the month series
NM=split(between(datetime(BR_MSCI_Market_D(2,1),'ConvertFrom','datenum'),datetime(BR_MSCI_Market_D(end,1),'ConvertFrom','datenum'), 'Months'), 'Months');
...
for t=2:size(BR_MSCI_Market_D,1)-1
    if month(datetime(BR_MSCI_Market_D(t,1),'ConvertFrom','datenum'))~=month(datetime(BR_MSCI_Market_D(t+1,1),'ConvertFrom','datenum')) % find end-of-month
        [N, S]=weekday(datetime(BR_MSCI_Market_D(t,1),'ConvertFrom','datenum'));
        j=0;
        switch N
            case 1 % Sunday
                j=-2; % use Friday (two days earlier)
...
% =======
% Transform all series into end-of-the month series
NM=split(between(datetime(MSCI_Market_D(2,1),'ConvertFrom','datenum'),datetime(MSCI_Market_D(end,1),'ConvertFrom','datenum'), 'Months'), 'Months');
...
for t=2:size(MSCI_Market_D,1)-1
    if month(datetime(MSCI_Market_D(t,1),'ConvertFrom','datenum'))~=month(datetime(MSCI_Market_D(t+1,1),'ConvertFrom','datenum')) % find end-of-month
        [N, S]=weekday(datetime(MSCI_Market_D(t,1),'ConvertFrom','datenum'));
        j=0;
        switch N
            case 1 % Sunday
                j=-2; % use Friday (two days earlier)
...
% SAVE .MAT
% ============================================= %
save(strcat(newpath,'BR_MSCI_Market_D.mat'),'BR_MSCI_Market_D');
save(strcat(newpath,'BR_MSCI_Market_dM.mat'),'BR_MSCI_Market_dM');
save(strcat(newpath,'Barclays_MSCI_Market_D.mat'),'Barclays_MSCI_Market_D');
save(strcat(newpath,'Barclays_MSCI_Market_dM.mat'),'Barclays_MSCI_Market_dM');
save(strcat(newpath,'MSCI_Market_dM.mat'),'MSCI_Market_dM');
save(strcat(newpath,'MSCI_Growth_dM.mat'),'MSCI_Growth_dM');
save(strcat(newpath,'MSCI_Value_dM.mat'),'MSCI_Value_dM');

save(strcat(newpath,'MSCI_Market_D.mat'),'MSCI_Market_D');
save(strcat(newpath,'MSCI_Growth_D.mat'),'MSCI_Growth_D');
save(strcat(newpath,'MSCI_Value_D.mat'),'MSCI_Value_D');
save(strcat(newpath,'MSCI_countries.mat'),'MSCI_countries');
...
% Save
% =======
save(strcat(newpath,'MSCI_Market_M_vol.mat'),'MSCI_Market_M_vol');
save(strcat(newpath,'Barclays_MSCI_Market_M_vol.mat'),'Barclays_MSCI_Market_M_vol');
save(strcat(newpath,'BR_MSCI_Market_M_vol.mat'),'BR_MSCI_Market_M_vol');

save(strcat(newpath,'MSCI_Market_M_sr.mat'),'MSCI_Market_M_sr');
save(strcat(newpath,'Barclays_MSCI_Market_M_sr.mat'),'Barclays_MSCI_Market_M_sr');
save(strcat(newpath,'BR_MSCI_Market_M_sr.mat'),'BR_MSCI_Market_M_sr');

save(strcat(newpath,'MSCI_Market_M_corr.mat'),'MSCI_Market_M_corr');
save(strcat(newpath,'Barclays_MSCI_Market_M_corr.mat'),'Barclays_MSCI_Market_M_corr');
save(strcat(newpath,'BR_MSCI_Market_M_corr.mat'),'BR_MSCI_Market_M_corr');
