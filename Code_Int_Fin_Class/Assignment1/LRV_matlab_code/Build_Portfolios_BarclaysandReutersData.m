...
% Program : 
% 1 ) Import Barclays and Reuters monthly data on spot and forward exchange rates, select subsample
...
% Take the end of the previous month as end-date
if ismac
...
% Import data downloaded from Datastream (Original freqency = daily, see prgm Merge_Barclays_Reuters_D)    
...
load(strcat(newpath,'BR_Spot_dM.mat'));                       %FX_Spot
FX_Spot=BR_Spot_M;
load(strcat(newpath,'BR_Fwd_EB_dM.mat'));                     %FX_Fwd Bid
FX_Fwd_EB=BR_Fwd_EB_M;
load(strcat(newpath,'BR_Spot_EB_dM.mat'));                    %FX_Spot Bid
FX_Spot_EB=BR_Spot_EB_M;
load(strcat(newpath,'BR_Fwd_EO_dM.mat'));                     %FX_Fwd Ask
FX_Fwd_EO=BR_Fwd_EO_M;
load(strcat(newpath,'BR_Spot_EO_dM.mat'));                    %FX_Spot Ask
FX_Spot_EO=BR_Spot_EO_M;
...
% Log excess return= log(F_ask(t)/S_bid(t+1));short in foreign, long in
% dollar
...
Track_portfolios(:,1)           =   FX_spot_chge_smple(:,1);    % Copy dates
        ...
        if n==number_portfolios                % Special case: last portfolio
            for m=1+(n-1)*floor(Count_Returns(t)*(1/number_portfolios)):n*floor(Count_Returns(t)*(1/number_portfolios))+rem(Count_Returns(t),number_portfolios)                   
                if m<=Count_Returns(t)
                    ...
                    Track_portfolios(t,1+Track_countries(t,1+m))      =   n;
...
% Excess returns
% lgsh=NaN*zeros(size(portfolios_xret_with_dates,1),size(portfolios_xret_with_dates,2)-2);
...
% Excess returns (with Bid/Ask Spreads)
% lgsh=NaN*zeros(size(portfolios_xretba_with_dates,1),size(portfolios_xretba_with_dates,2)-2);
...
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
% Tables (monthly series)
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
disp(' ');
disp('Tables (monthly series, annualized, estimation period)');
...
% SAVE PORTFOLIOS
% ============================================= %
portfolios_Spotchg_BarclaysandReuters   = portfolios_Spotchg_with_dates;
portfolios_FXXR_BarclaysandReuters      = portfolios_FXXR_with_dates;
portfolios_Dsct_BarclaysandReuters      = portfolios_Dsct_with_dates;
portfolios_xret_BarclaysandReuters      = portfolios_xret_with_dates;           % Excess return= F_mid(t)/S_mid(t+1)-1 
portfolios_xretba_BarclaysandReuters    = portfolios_xretba_with_dates;         % Excess return= F_ask(t)/S_bid(t+1)-1 or excess return= F_bid(t)/S_ask(t+1)-1
portfolios_logxret_BarclaysandReuters   = portfolios_logxret_with_dates;        % Log excess return= log(F_mid(t)/S_mid(t+1))
portfolios_logxretba_BarclaysandReuters = portfolios_logxretba_with_dates;      % Log excess return= log(F_ask(t)/S_bid(t+1)) or Log excess return= log(F_bid(t)/S_ask(t+1));;

save(strcat(newpath,'portfolios_Spotchg_BarclaysandReuters.mat'),'portfolios_Spotchg_BarclaysandReuters');
save(strcat(newpath,'portfolios_FXXR_BarclaysandReuters.mat'),'portfolios_FXXR_BarclaysandReuters');
save(strcat(newpath,'portfolios_Dsct_BarclaysandReuters.mat'),'portfolios_Dsct_BarclaysandReuters');
save(strcat(newpath,'portfolios_xret_BarclaysandReuters.mat'),'portfolios_xret_BarclaysandReuters');
save(strcat(newpath,'portfolios_xretba_BarclaysandReuters.mat'),'portfolios_xretba_BarclaysandReuters');

save(strcat(newpath,'portfolios_xretba_BarclaysandReuters.mat'),'portfolios_xretba_BarclaysandReuters');
save(strcat(newpath,'portfolios_logxret_BarclaysandReuters.mat'),'portfolios_logxret_BarclaysandReuters');
save(strcat(newpath,'portfolios_logxretba_BarclaysandReuters.mat'),'portfolios_logxretba_BarclaysandReuters');

save(strcat(newpath,'Track_portfolios_Carry.mat'),'Track_portfolios');
